...
from notecast.core.models import User, Job, Episode # Assuming these models exist
...
class SQLiteJobRepository(JobRepository):
    ...
    def create_job(self, user: User, episode: Episode) -> Job:
        ...
        Args:
            user: User object
            episode: Episode object to create job for
    ...
    async def get_next_pending(self, user: User) -> Optional[Job]:
        await asyncio.sleep(0) # Simulate async DB operation
        with self._conn() as conn:
            row = conn.execute(
